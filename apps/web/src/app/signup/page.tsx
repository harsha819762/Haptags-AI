...
export default function SignupPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setToken(res.token);
      setStoredUser({ email: res.user.email, organizationName: res.organization.name });
      router.push("/app");
    } catch (err) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <h1 className="font-serif font-bold text-2xl mt-6 mb-1">Create your account</h1>
        <p className="text-sm text-[#46566A] mb-6">Start with 100 free credits.</p>
        ...
        <p className="text-sm text-[#46566A] mt-6">
          ...
          <Link href="/login" className="text-[#2A78D6] font-medium">
            Log in
          </Link>
        </p>
      </div>
